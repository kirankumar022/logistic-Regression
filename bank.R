...
# Confusion Matrix
confusionMatrix(factor(claimants$y, levels = c(0, 1)), factor(pred_values, levels = c(0, 1)))
...
# The output of sigmoid function lies in between 0-1
fullmodel <- glm(y ~ ., data = claimants1, family = "binomial")
summary(fullmodel)
...
optCutOff <- optimalCutoff(claimants1$Result, prob_full)
optCutOff
...
misClassError(claimants1$y, prob_full, threshold = optCutOff)


# ROC curve
# Greater the area under the ROC curve, better the predictive ability of the model
plotROC(claimants1$y, prob_full)
